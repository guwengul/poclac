...
export default async function CalibrationPeriodPage({
  ...
  // Find tribes that are in CALIBRATION status for this period
  const calibrationTribeIds = auth.isAdmin
    ? (await prisma.tribePeriod.findMany({
        where: { periodId, status: "CALIBRATION" },
        select: { tribeId: true },
      })).map(t => t.tribeId)
    : (await prisma.tribePeriod.findMany({
        where: { periodId, status: "CALIBRATION", tribeId: { in: auth.hrTribeIds } },
        select: { tribeId: true },
      })).map(t => t.tribeId);

  // Also include ACTIVE tribes for admins (they can calibrate anytime)
